...
async fn main() -> anyhow::Result<()> {
    ...
    // Add the example-specific import (print function)
    // The component bindgen generates add_to_linker for world-level functions
    linker.instance("example")?.func_wrap(
        "print",
        |mut _caller: wasmtime::StoreContextMut<'_, HostState>,
         (s,): (String,)|
         -> wasmtime::Result<()> {
            println!("{}", s);
            Ok(())
    ...
    let instance = Example::instantiate(&mut store, &component, &linker).unwrap();
